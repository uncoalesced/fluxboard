 ...
 */
@Singleton
class AndroidAnimatedStickerConverter
    @Inject
    constructor() : AnimatedStickerConverter {
                        ...
                        if (rawBmp != null) {
                            ...
                            if (targetFormat == "image/gif") {
                                ...
                                gifEncoder?.writeFrame(
                                    ...
                                    width,
                                    height,
                                    intervalMs.toInt().milliseconds,
                                )
                            } else if (targetFormat == "image/webp") {
                                val encoder =
                                    webpEncoder ?: WebPAnimEncoder(context, width, height).also {
                                        webpEncoder = it
                                    }
                    ...
                    if (targetFormat == "image/gif") {
                        ...
                        try {
                            ...
                            if (isValidAnimatedWebP(webpBytes)) {
                                ...
                                Result.failure(
                                    IllegalStateException(
                                        "Encoder output is not a valid animated WebP",
                                    ),
                                )
                            }
                        } finally {
        ...
        companion object {
            ...
            private fun fourCC(
                bytes: ByteArray,
                offset: Int,
            ): String = String(bytes, offset, 4, Charsets.US_ASCII)
