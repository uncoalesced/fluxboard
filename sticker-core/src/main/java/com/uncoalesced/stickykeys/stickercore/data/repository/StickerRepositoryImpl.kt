...
class StickerRepositoryImpl
    ...
    constructor(
        ...
        override fun getStickersByCategory(categoryId: String): Flow<List<Sticker>> =
            stickerDao.getStickersByCategory(categoryId).map { list ->
                list.map {
                    it.toDomain()
                }
            }
        ...
        override fun getFavouriteStickers(): Flow<List<Sticker>> =
            stickerDao.getFavouriteStickers().map { list ->
                list.map {
                    it.toDomain()
                }
            }
        ...
        override suspend fun updateStickerMetadata(sticker: Sticker) =
            withContext(Dispatchers.IO) {
                stickerDao.updateSticker(sticker.toEntity())
            }

        override suspend fun deleteSticker(id: String) =
            withContext(Dispatchers.IO) {
                // Delete from disk
