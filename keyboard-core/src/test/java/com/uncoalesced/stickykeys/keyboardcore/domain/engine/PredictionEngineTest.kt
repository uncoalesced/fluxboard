...
class PredictionEngineTest {
    ...
    @Test
    fun testDictionaryLoadedAndReturnsSuggestions() =
        runBlocking {
    ...
    @Test
    fun testPersonalDictionaryLearning() =
        runBlocking {
            ...
            assertTrue(
                "A word used only once should not be suggested yet",
                !afterOneUse.contains(uniqueWord),
            )
    ...
    @Test
    fun testCorpusTyposAreNotTreatedAsWords() =
        runBlocking {
            // These were all terminal entries in the unfiltered dictionary (freq 134-153),
            // which made the engine refuse to correct them: a word already in the
            // dictionary is by definition not a typo. The build-time SCOWL intersection
            // in dictionary-tools/build_flictionary.py is what removes them, so this test
            // fails if base_dict.bin is ever regenerated without the --wordlist filter.
            // Transpositions have exactly one sensible target, so assert it exactly.
            mapOf(
                "teh" to "the",
                "hte" to "the",
                "adn" to "and",
                ...
                assertEquals(
                    "'$typo' should correct to '$expected'",
                    expected,
                    engine.getAutoCorrection(typo),
                )
            }

            // "helo" is one edit from both "hello" and "help"; which wins is a frequency
            // call, not a correctness one. What matters is that it is corrected at all --
            // as a dictionary terminal it used to be left alone.
            listOf("helo", "wrod").forEach { typo ->
                val correction = engine.getAutoCorrection(typo)
                assertNotNull("'$typo' should be corrected, not accepted as a word", correction)
                assertNotEquals("'$typo' should not correct to itself", typo, correction)
            }

            // The real words those typos target must survive the filter untouched.
            listOf("the", "hello", "and", "receive", "that").forEach { word ->
                assertNull(
