...
class LinkSharingManager
    ...
    constructor(
        ...
        companion object {
            /**
             * Field separator inside the link fragment.
             *
             * Must not occur in any field it separates: the base64url key alphabet is
             * A-Z a-z 0-9 plus '-', '_' and '=' padding, session ids are UUIDs (hex and
             * '-'), and the host/port are digits with '.' or ':'. '~' is URI-unreserved
             * (RFC 3986) and appears in none of them. Splitting on '_' previously broke
             * whenever the random key happened to contain one.
             */
            const val DELIMITER = "~"
        }
         ...
         */
        suspend fun prepareShareLink(): ShareContext =
            withContext(Dispatchers.IO) {
                ...
                val link = "https://stickykeys.app/s#$sessionId$DELIMITER$base64Key$DELIMITER$ip$DELIMITER$port"
         ...
         */
        suspend fun connectToLink(linkUrl: String): ReceiveContext =
            withContext(Dispatchers.IO) {
                val fragment = linkUrl.substringAfterLast("#", "")
                if (fragment.isEmpty()) throw IllegalArgumentException("Invalid link format")

                val parts = fragment.split(DELIMITER)
                // Either session+key, or session+key+ip+port. Any other shape is malformed.
                if (parts.size != 2 &&
                    parts.size != 4
                ) {
                    throw IllegalArgumentException("Invalid link format")
                }
