...
class MigrationPackager
    ...
    constructor(
         ...
         */
        suspend fun extractDataFromStream(input: InputStream) =
            withContext(Dispatchers.IO) {
                            ...
                            when {
                                name.startsWith("databases/") ->
                                    resolveWithinBase(
                                        ...
                                        name.removePrefix("databases/"),
                                    )
                                name.startsWith("datastore/") ->
                                    resolveWithinBase(
                                        File(context.filesDir, "datastore"),
                                        name.removePrefix("datastore/"),
                                    )
                                name.startsWith("stickers/") ->
                                    resolveWithinBase(
                                        File(context.filesDir, "stickers"),
                                        name.removePrefix("stickers/"),
                                    )
                                name.startsWith("stickers_thumbnails/") ->
                                    resolveWithinBase(
                                        File(context.filesDir, "stickers_thumbnails"),
                                        name.removePrefix("stickers_thumbnails/"),
                                    )
                                else -> null
                            }
         ...
         */
        internal fun resolveWithinBase(
            baseDir: File?,
            relativePath: String,
        ): File? {
