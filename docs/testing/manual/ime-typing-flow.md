...
This script outlines the end-to-end journey for switching to the FluxBoard IME, typing, receiving predictions, and committing text to a target app. Full automation is impractical here because standard instrumented tests cannot seamlessly interact with system-level IME switching dialogs or simulate the IPC InputConnection boundary faithfully without root access.

## Prerequisites
1. Open the Android System Settings > System > Languages & Input > On-screen keyboard.
2. Enable **FluxBoard** and accept the system warning.
...
1. **Activate the IME**
   ...
   - Use the navigation bar keyboard switcher icon (or long-press spacebar on some ROMs) to switch the active keyboard to **FluxBoard**.
   - *Expected:* The FluxBoard custom UI appears.
...
5. **Auto-Capitalization**
   - Ensure Auto-Capitalization is enabled in FluxBoard Settings.
...
6. **Clipboard History Access**
   ...
   - Tap the Clipboard icon on the FluxBoard toolbar.
